...
label-1: add x11, x10, #0x1                          // i=i+1, i+1->x11
       	ldr	w12, [x3, x11, lsl #2]                   // row_ptr[i+1] -> w12, base addr of row_ptr is x3
       	...
       	sub	x15, x13, #0x1                           // row_ptr[i+1]-1 -> x15
...
label-3: ldursw	x17, [x16, #-256]                    // col_idx[j] -> x17
       	...
       	ldrsw	x18, [x4, x18, lsl #2]               // col_idx[pref_j] -> x18, base addr of col_idx[] = x4
            for (int j = last_row; j < this_row; j++)
                ret[i] += val[j] * vec[col_idx[j]];
            for (int j = last_row; j < this_row; j++)
                ret[i] += val[j] * vec[col_idx[j]];
      	...
      	add	x14, x14, #0x1                           // j = j + 1  
       	...
       	fadd	d0, d0, d1
       	cmp	x13, x14
       	add	x17, x1, x18, lsl #3                     // addr of vec[j+32]-> x17
