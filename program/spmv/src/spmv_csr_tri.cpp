...
//double vec_fake_bob[max_col];
//double vec_fake_peter[max_col];
...
void  mul(double *ret, double *vec, double *val, int *row_ptr, int *col_idx, 
            int *col_idx_fake, double *vec_fake_bob, double *vec_fake_peter)
{
        ...
        for (int i = 0; i < row; i++) {
            ...
            for (int j = last_row; j < this_row; j++) {
                ret[i] += val[j] * vec[col_idx[j]] + vec_fake_bob[col_idx[j]] + vec_fake_peter[col_idx_fake[j]];
...
{    
    ...
    //double vec_fake_bob[max_col];
    double * const vec_fake_bob = (double *)0xc4000000;
    //double vec_fake_peter[max_col];
    double * const vec_fake_peter = (double *)0xc8000000;
    ...
    if (strcmp(argv[1],"true")==0){
        ...
        for (int i=0;i<2;i++){
            mul(ret, vec, val, row_ptr, col_idx, 
                    col_idx_fake, vec_fake_bob, vec_fake_peter);
        }
    }
