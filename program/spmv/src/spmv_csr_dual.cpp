...
void  mul(double *ret, double *vec, double *val,int *row_ptr, int *col_idx, double *vec_fake) 
{
        ...
        for (int i = 0; i < row; i++) {
            ...
            for (int j = last_row; j < this_row; j++) {
                ret[i] += val[j] * vec[col_idx[j]] + vec_fake[col_idx[j]];
...
{    
    ...
    if (strcmp(argv[1],"true")==0){
        ...
        for (int i=0;i<2;i++){
            mul(ret, vec, val, row_ptr, col_idx, vec_fake);
        }
    }
