...
void BFS_kernel(int depth)
    ...
    adrp	x15, 5a67000 
    adrp	x16, 492000
    movk	w11, #0x98, lsl #16    // 0x98965f -> w11
    add	    x13, x13, #0x970
    add	    x14, x14, #0xb60
    add	    x15, x15, #0x568
    add	    x16, x16, #0xad0
  ...
  label-1: 	
    add	    x1, x13, x9, lsl #2    // addr of bfs_queue[queue_start_ptr] -> x1
   	...
   	cmp	    x9, x11                // compare queue_start_ptr with 0x98965f, 0x98965f is a parameter   
   	csel	x18, x9, x11, lt       // if queue_start_ptr < 0x98965f, pref_start_ptr = queue_start_ptr, pref_start_ptr -> x18
   	...
   	prfm	pldl1keep, [x1, #256]  // prefetch bfs_queue[queue_start_ptr+32], 256 is a parameter, prefetch for row_ptr[start]
  ...
  label-4: 	
    ldur	w3, [x1, #-256]        // col_idx[i] -> w3
   	cmp	    x18, x11               // compare i with 0x98965f
   	csel	x4, x18, x11, lt       // if i < 0x98965f, pref_i = i, pref_i -> x4
